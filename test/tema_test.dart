...
void main() {
  ...
  group('contraste das duas paletas', () {
    ...
    for (final brilho in Brightness.values) {
      ...
      test(
        '$nome: o botão de voz ativo tem o metal legível sobre o próprio '
        'fundo',
        () {
          // O estado ativo do botão de ouvir usa o anel do metal (ícone e
          // rótulo em primary) sobre surfaceContainerHighest. Se alguém
          // trocar o par por um tom sem contraste, o teste avisa.
          final cor = construirTema(brilho: brilho).colorScheme;
          expect(
            contraste(cor.primary, cor.surfaceContainerHighest),
            greaterThan(4.5),
          );
        },
      );
  ...
  group('escala do texto de leitura', () {
    ...
    test(
      'a escala multiplica integralmente a leitura e com teto de 1,3x o resto',
      () {
        // Regra da Escala (DESIGN.md): só bodyLarge/bodyMedium escalam
        // integralmente — título, rótulo e legenda escalam a 30% do pedido,
        // com teto de 1,3x, para não estourar AppBar/Nav.
        ...
        expect(
          grande.labelMedium?.fontSize,
          closeTo(padrao.labelMedium!.fontSize! * limitada, 0.001),
        );
        expect(
          grande.headlineMedium?.fontSize,
          closeTo(padrao.headlineMedium!.fontSize! * limitada, 0.001),
        );
        expect(
          grande.titleSmall?.fontSize,
          closeTo(padrao.titleSmall!.fontSize! * limitada, 0.001),
        );
      },
    );

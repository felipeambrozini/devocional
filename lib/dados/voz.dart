...
class VozExcecao implements Exception {
  const VozExcecao(this.mensagem);
...
class Voz extends ChangeNotifier {
  ...
  Future<void> alternar(String chave) async {
    ...
    if (url == null) {
      _carregando = false;
      notifyListeners();
      throw const VozExcecao('Áudio ainda não disponível para esta leitura.');
    }
  ...
  /// Toca com a guarda padrão das leituras: [VozExcecao] passa como veio,

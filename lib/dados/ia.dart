...
///
/// A chave mora em `lib/dados/google.dart`, junto do cabeçalho de identidade do
/// apk. A leitura em voz alta não usa mais essa chave nem TTS: os áudios são
/// MP3 pré-gerados (ver `lib/dados/voz.dart`).
///
...
class IaExcecao implements Exception {
  const IaExcecao(this.mensagem);
...
Future<String> perguntar({
  ...
  if (resposta.statusCode != 200) {
    ...
    throw IaExcecao(_mensagemDeErro(resposta));
  }
  ...
  if (texto is! String || texto.trim().isEmpty) {
    throw const IaExcecao('A resposta veio vazia. Tente de novo.');
  }
  return texto.trim();
}
